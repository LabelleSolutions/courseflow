 ...
 *
 * Returns an error message string when the value is absent, not a string, or
 * blank; returns null when the value is valid. Extracted to eliminate the
 * repeated inline validation blocks for each required string field.
 *
 * @param {unknown} value
...
router.post('/analyze-speech', async (req, res) => {
  try {
    ...
    const transcriptError = requireNonEmptyString(transcript, 'transcript');
    if (transcriptError) {
      return res.status(400).json({ success: false, error: transcriptError });
    }
    const targetSentenceError = requireNonEmptyString(targetSentence, 'targetSentence');
    if (targetSentenceError) {
      return res.status(400).json({ success: false, error: targetSentenceError });
    }
